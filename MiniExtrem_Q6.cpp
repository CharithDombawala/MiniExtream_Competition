...
int quickestWayUp(vector<vector<int>>& ladders, vector<vector<int>>& snakes) {
    ...
    queue<int> q;
    vector<int> distance(numSquares + 1, -1);
    ...
    while (!q.empty()) {
        ...
        if (currentSquare == numSquares) {
            return distance[currentSquare];
        }

        for (int i = 1; i <= numFaces; i++) {
            int nextSquare = currentSquare + i;
            if (nextSquare <= numSquares) {
               
                if (ladderMap.count(nextSquare)) {
                    nextSquare = ladderMap[nextSquare];
                } else if (snakeMap.count(nextSquare)) {
                    nextSquare = snakeMap[nextSquare];
                }

                if (distance[nextSquare] == -1) {
                    distance[nextSquare] = distance[currentSquare] + 1;
                    q.push(nextSquare);
                }
            }
        }
